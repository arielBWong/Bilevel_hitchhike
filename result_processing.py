...
def combine_hv_igd_out(methods, seeds, problems, foldername):
    ...
    colume_name = []
    for i, method in enumerate(methods):
        colume_name.append(methods_[3 * i] + '-median')
        colume_name.append(methods_[3 * i+1] + '-mean')
        colume_name.append(methods_[3 * i+2] + '-ci')


    h = pd.DataFrame(output_matrix_h, columns=colume_name, index=index_seeds)
    g = pd.DataFrame(output_matrix_g, columns=colume_name, index=index_seeds)
