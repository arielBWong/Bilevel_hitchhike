...
def expected_improvement(x,
    ...
    # predict f
    mu, sigma = predictions(x, krg)
    # change to matrix calculation
    pf = np.atleast_2d(np.ones((n_samples, 1)))

    if n_g > 0:
        # so/mo with constraint
        mu_gx, sigma_gx = predictions(x, krg_g)

        with np.errstate(divide='warn'):
            pf_m = gaussiancdf((0 - mu_gx)/sigma_gx)
        ...
        # single objective without constraint
        mu_sample_opt = np.min(norm_train_y, axis=0)
    ...
    if n_obj > 1:
        ...
        if n_g > 0:
            ...
            if len(feasible) > 1:
                # feasible passed-in is in fact norm-feasible-nd
                point_reference = np.atleast_2d([1.1] * n_obj)
                ei = EIM_hv(mu, sigma, feasible, point_reference)
            else:
                return pf
        else:
